...
class IosImageConverter : ImagesUtils {
    @OptIn(ExperimentalForeignApi::class)
    override fun imageConverter(imageBytes: ByteArray): ImageBitmap {
        val nsData = imageBytes.toData()
        // Step 2: Convert NSData to UIImage
        val uiImage = UIImage.imageWithData(nsData)// Handle error case if UIImage cannot be created

        // Step 3: Extract CGImage from UIImage

        // Step 4: Convert CGImage to ImageBitmap
