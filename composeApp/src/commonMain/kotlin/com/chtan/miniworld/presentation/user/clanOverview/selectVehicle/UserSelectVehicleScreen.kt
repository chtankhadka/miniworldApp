...
import com.chtan.miniworld.presentation.user.clanOverview.UserClanOverviewRoute
import com.chtan.miniworld.presentation.user.dashboard.map.ObjectDetails
...
fun UserSelectVehicleScreen(
    ...
    Column(
        modifier = Modifier
            .background(color = MaterialTheme.colorScheme.outline).padding(2.dp)
    ) {
        Row(modifier = Modifier.weight(1f)) {
            ...
            if (selectedButton == UserSelectVehicleButtonType.MapOverview)
                MiniWorldPager(
                    ...
                    state.mapDetails.map { it.id to it.img })
            else
                TestDrawablePager(
                    modifier = Modifier.fillMaxWidth(0.7f),
                    state.vehicleDetails.map { it.id to it.img })
            //Details
        ...
        Row(
            ...
            listOf(
                ...
                UserSelectVehicleButtonType.Continue,
            ).forEach { buttonType ->
                Button(
                    ...
                    onClick = {
                        ...
                        when (buttonType) {
                            ...
                            UserSelectVehicleButtonType.Continue -> {
                                nav.navigate(UserClanOverviewRoute.StartOverview)
                            }
                        }
